...
( function(){
	...
	function _search( options ){
        ...
        // ------------------------------
        if( options.env ){
            if( debug ) console.log( 'CONFIG: looking for env variable: ', options.env );
            var envFromEnvVar = process.env[ options.env];
            if( envFromEnvVar ){
                if( debug ) console.log( 'CONFIG: loaded environment from env variable: ', options.env, '=', envFromEnvVar );
		...
		if( options.paths ) {
            ...
            if (envFromFile !== false) {
                ...
                if( debug ) console.log( 'CONFIG: loaded environment from file:', envFromFile );
                return envFromFile;
            }
        }
		...
		if( defaultEnvironment ){
            if( debug ) console.log( 'CONFIG: using default environment:', defaultEnvironment );
        ...
        // ------------------------------
        if( debug ) console.log( 'CONFIG: no environment found' );
		return false;
	}
