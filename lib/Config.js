...
( function() {
    ...
    function Config( options ){
	    ...
	    this._options.maxValueLength = 60;
    }
    ...
    Config.prototype.list = function(){
	    ...
	    var dStr = '|--';
	    var depthString = dStr+dStr+dStr+dStr+dStr+dStr+dStr+dStr+dStr+dStr+dStr+dStr+dStr;
	    ...
	    function _recursiveListObject( object, path, depth ){
            ...
            for( var key in object ){
                if( object.hasOwnProperty( key ) ){
	                var shallRecurse = false;
	                
                    var line = ' ' + chalk.white( depthIndicator );
	                
	                if( depth > 0 ) line += ' ';
	                
	                line += chalk.yellow( key ) + chalk.white(' : ');
	                ...
	                if( value instanceof Object ){
		                if( _isPrintableObject( value ) ){
			                ...
			                line += chalk.green( value.toString().substring( 0, self._options.maxValueLength) ) + ' ';
		                } else {
			                shallRecurse = true;
		                }
	                } else {
		                line += chalk.green( value.toString( 0, self._options.maxValueLength ) ) + ' ';
	                }
    ...
    Config.prototype._merge = function( backObject, frontObject, sourceName, currentPath ){
        ...
        if(  Array.isArray( frontObject ) || !(frontObject instanceof Object) ){       // primitive value returned as-is, array used without merging
            return frontObject;
        }
